...
\begin{enumerate}
  ...
  \item \dd{details}: Arbitrary text, matching \dd{/[a-zA-Z0-9 -.]\{1,128\}/};
...
The most active node with the highest score and the highest availability
will be selected as tax receiver. It's up to the payer which node to select,
but the built-in mechanism pays attention to the following factors (in order
of importance):
...
payments are ignored. Also, to prevent multiple use of the same score,
all incoming tax payments must have unique \dd{details}.
