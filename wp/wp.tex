...
promote it further, which will seriously decrease wallet visibility in
the network---only a limited amount of nodes will see it, unless the owner
manually ``push'' it to many nodes, which is a time and network traffic
expensive operation.
...
correct one, transaction by transaction. If a transaction already exists,
it's ignored. If a transaction is negative (spending money) and its ID is lower than
the maximum ID in the ledger, it gets ignored as a fraudulent one
(``double spending''). Otherwise, it gets to the end of the ledger.

Then, when merge is done, the modifications get \emph{propagated} to other wallets
available locally. Each transaction that has a negative amount is copied
added to the ledger of their receiving wallets, if they don't yet exist there.
