...
class TestRemote < Zold::Test
  def test_updates_remote
    Dir.mktmpdir do |dir|
      ...
      stub_request(:get, 'http://localhost:2/remotes').to_return(status: 404)
      stub_request(:get, 'http://localhost:888/remotes').to_return(status: 404)
      stub_request(:get, 'http://localhost:999/remotes').to_return(status: 404)
  ...
  def test_new_version_rubygems
    Dir.mktmpdir do |dir|
      ...
      log = TestLogger.new
      cmd = Zold::Remote.new(remotes: Zold::Remotes.new(file: File.join(dir, 'remotes.txt')), log: log)
      cmd.run(%w[remote clean])
      cmd.run(['remote', 'add', zero.host, zero.port.to_s, '--skip-ping'])
  ...
  def test_elects_a_remote
    Dir.mktmpdir do |dir|
      ...
      stub_request(:get, "http://#{zero.host}:#{zero.port}/").to_return(
        status: 200,
        body: { version: Zold::VERSION, score: zero.to_h }.to_json
      )
      assert_equal(
        1,
        Zold::Remote.new(remotes: remotes, log: fake_log).run(%w[remote elect --ignore-score-value]).count
      )
    end
  end
  ...
  def test_remote_trim_with_tolerate
    Dir.mktmpdir do |dir|
      remotes = Zold::Remotes.new(file: File.join(dir, 'remotes.txt'))
      score = Zold::Score.new(host: 'aa1.example.org', port: 9999, invoice: 'NOPREFIX4@ffffffffffffffff')
      stub_request(:get, 'http://localhost:8883/version').to_return(status: 200, body: '0.0.0')
      stub_request(:get, "http://#{score.host}:#{score.port}/remotes").to_return(
        status: 200,
        body: { version: Zold::VERSION, score: score.to_h, all: [{ host: 'localhost', port: 8883 }] }.to_json
      )
      stub_request(:get, 'https://rubygems.org/api/v1/versions/zold/latest.json').to_return(
        status: 200,
        body: '{"version": "0.0.0"}'
  ...
  def test_select_selects_the_strongest_nodes
    Dir.mktmpdir do |dir|
      ...
      assert_equal([7, 8, 9, 10, 11], remotes.all.map { |r| r[:score] }.sort)
    end
  end
  ...
  def test_select_respects_max_nodes_option
    Dir.mktmpdir do |dir|
      ...
      (5000..5010).each do |port|
        stub_request(:get, "http://#{zero.host}:#{zero.port}/remotes").to_return(
          status: 200,
          body: { version: Zold::VERSION, score: zero.to_h, all: [{ host: 'localhost', port: port }] }.to_json
        )
        stub_request(:get, "http://localhost:#{port}/version").to_return(
          status: 200,
          body: { version: Zold::VERSION }.to_json
        )
  ...
  def test_sets_masters
    Dir.mktmpdir do |dir|
      remotes = Zold::Remotes.new(file: File.join(dir, 'remotes.txt'))
      Zold::Remote.new(remotes: remotes, log: fake_log).run(%w[remote masters])
  ...
  def test_updates_just_once
    Dir.mktmpdir do |dir|
      ...
      get = stub_request(:get, "http://#{zero.host}:#{zero.port}/remotes").to_return(
        ...
        status: 200,
        headers: {},
        body: { version: Zold::VERSION, score: zero.to_h, all: [] }.to_json
      )
      cmd = Zold::Remote.new(remotes: remotes, log: fake_log)
      cmd.run(['remote', 'add', zero.host, zero.port.to_s, '--skip-ping'])
