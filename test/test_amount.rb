...
class TestAmount < Zold::Test
  def test_parses_zld
    ...
    assert_includes(amount.to_s, '14.95ZLD', "#{amount} is not equal to '14.95ZLD'")
  end
  ...
  def test_parses_zents
    ...
    assert_includes(amount.to_s, '0.21ZLD', "#{amount} is not equal to '0.21ZLD'")
  end

  def test_compares_amounts
    amount = Zold::Amount.new(zents: 700_000_000)
    assert_operator(amount, :>, Zold::Amount::ZERO, "#{amount} is not greater than zero")
  end
