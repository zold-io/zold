  ...
  class Merge
      ...
      opts =
        Slop.parse(args, help: true, suppress_errors: true) do |o|
          o.banner = <<~BANNER
            ...
            Available options:
          BANNER
          o.bool('--skip-propagate', 'Don\'t propagate after merge', default: false)
          o.bool('--skip-legacy', 'Don\'t make legacy transactions (older than 24 hours) immutable', default: false)
          o.bool('--quiet-if-absent', 'Don\'t fail if the wallet is absent', default: false)
          o.integer(
            '--depth',
          ...
          o.bool('--no-baseline', 'Don\'t treat the highest score master copy as trustable baseline', default: false)
          o.bool(
            '--edge-baseline',
          ...
          o.integer('--trusted-max', 'The maximum amount of trusted wallets we can see in the list', default: 128)
    ...
    def merge(id, cps, opts)
      ...
      cps.each_with_index do |c, idx|
        ...
        apply(
          opts, patch, Wallet.new(c[:path]), "#{c[:name]}/#{idx}/#{c[:score]}#{'/baseline' if baseline}",
          baseline: baseline, master: c[:master]
        )
        score += c[:score]
      end
      @wallets.acq(id) do |w|
      ...
      if patch.empty?
        ...
        raise(RuntimeError, "There are no copies of #{id}, nothing to merge")
      end
      modified =
      ...
      if modified
        @log.info(
          "#{cps.count} copies with the total score of #{score} successfully merged " \
    ...
    def apply(opts, patch, wallet, name, baseline: false, master: false)
      @log.debug("Adding copy ##{name}#{' (master)' if master} to the patch #{wallet.mnemo}...")
      if opts['depth'].positive?
        patch.join(wallet, ledger: opts['ledger'], baseline: baseline, master: master) do |txn|
          ...
          if trusted.include?(txn.bnf.to_s)
            ...
            @log.debug(
              "Won't PULL #{txn.bnf} since there are too many trusted wallets already: " \
      ...
      @log.debug("Copy ##{name} of #{wallet.id} merged in #{Age.new(Time.now)}: #{patch}")
