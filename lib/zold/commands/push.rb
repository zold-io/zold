...
module Zold
  ...
  class Push
    ...
    def run(args = [])
      opts =
        Slop.parse(args, help: true, suppress_errors: true) do |o|
          ...
          o.bool('--quiet-if-missed', 'Don\'t fail if the wallet wasn\'t delivered to any remotes', default: false)
          o.array('--ignore-node', 'Ignore this node and don\'t push to it', default: [])
          o.integer('--threads', 'How many threads to use for pushing wallets (default: 1)', default: 1)
          o.integer(
            '--retry',
    ...
    def push(id, opts)
      raise(RuntimeError, "There are no remote nodes, run 'zold remote reset'") if @remotes.all.empty?
      @wallets.acq(id) do |wallet|
        raise(RuntimeError, "The wallet #{id} is absent at #{wallet.path}") unless wallet.exists?
      end
      ...
      @remotes.iterate(@log) do |r|
        nodes.increment
        total.increment(upload(id, r, opts))
      ...
      unless opts['quiet-if-missed']
        if done.value.zero?
          raise(
            RuntimeError,
            "No nodes out of #{nodes.value} accepted the wallet #{id}; run 'zold remote update' and try again"
          )
        end
        ...
        if nodes.value < opts['tolerate-quorum']
          raise(
            NoQuorum,
            "There were not enough nodes, the required quorum is #{opts['tolerate-quorum']}; " \
            "run 'zold remote update' or use --tolerate-quorum=1"
          )
        end
      end
      @log.info(
        "Push finished to #{done.value - masters.value}+#{masters.value}m nodes " \
        "out of #{nodes.value} in #{Age.new(Time.now)}, total score for #{id} is #{total.value}"
      )
    end

    def upload(id, r, opts)
      if opts['ignore-node'].include?(r.to_s)
        @log.debug("#{r} ignored because of --ignore-node")
        return 0
      end
      start = Time.now
      read(id, r, opts) do |json, score|
        r.assert_valid_score(score)
        r.assert_score_ownership(score)
        r.assert_score_strength(score) unless opts['ignore-score-weakness']
        @log.debug(
          "#{r} accepted #{@wallets.acq(id, &:mnemo)} in #{Age.new(start, limit: 4)}: " \
    ...
    def read(id, r, opts)
      ...
      begin
        ...
        if response.status == 304
          @log.debug(
            "#{r}: same version of #{@wallets.acq(id, &:mnemo)} there, didn't push " \
