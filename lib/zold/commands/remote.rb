  ...
  class Remote
    ...
    def run(args = [])
      opts =
        Slop.parse(args, help: true, suppress_errors: true) do |o|
          ...
          o.bool('--ignore-score-value', 'Don\'t complain when their score is too small', default: false)
          o.array('--ignore-node', 'Ignore this node and never add it to the list', default: [])
          o.bool(
            '--ignore-if-exists',
            'Ignore the node while adding if it already exists in the list',
            default: false
          )
          o.bool('--ignore-masters', 'Don\'t elect master nodes, only edges', default: false)
          o.bool('--masters-too', 'Give no privileges to masters, treat them as other nodes', default: false)
          o.integer(
            '--min-score',
            "The minimum score required for winning the election (default: #{Tax::EXACT_SCORE})",
            default: Tax::EXACT_SCORE
          )
          o.integer('--max-winners', 'The maximum amount of election winners the election (default: 1)', default: 1)
          o.integer(
            '--retry',
            'How many times to retry each node before reporting a failure (default: 2)',
            default: 2
          )
          o.bool('--skip-ping', 'Don\'t ping back the node when adding it (not recommended)', default: false)
          o.bool('--ignore-ping', 'Don\'t fail if ping fails, just report the problem in the log', default: false)
          o.integer(
            '--depth',
          ...
          o.bool('--reboot', 'Exit if any node reports version higher than we have', default: false)
      ...
      command = mine.first
      raise(RuntimeError, "A command is required, try 'zold remote --help'") unless command
      case command
      when 'show'
        show
      when 'clean'
      ...
      when 'masters'
        masters(opts)
      when 'add'
        add(mine[1], mine[2] ? Integer(mine[2], 10) : Remotes::PORT, opts)
      when 'remove'
        remove(mine[1], mine[2] ? Integer(mine[2], 10) : Remotes::PORT, opts)
      when 'elect'
        elect(opts)
      when 'trim'
        trim(opts)
    ...
    def show
      @remotes.all.each do |r|
        @log.info(
          [
            "#{r[:host]}:#{r[:port]}#{Rainbow("/#{r[:score]}").color(r[:score].positive? ? :green : :red)}",
    ...
    def clean
      before = @remotes.all.count # rubocop:disable Elegant/NoRedundantVariable
      @remotes.clean
    ...
    def trim(opts)
      ...
      @log.info(
        "The list of #{all.count} remotes trimmed down to #{@remotes.all.count} nodes " \
    ...
    def update(opts)
      ...
      max = capacity.map { |c| c[:count] }.max || 0
      capacity.each do |c|
        @remotes.error(c[:host], c[:port]) if c[:count] < max
      end
    ...
    def refresh(r, opts)
      ...
      begin
        ...
        yield(json, Score.parse_json(json['score']))
    ...
    def reboot(r, json, opts)
      ...
      if mine < Semantic::Version.new(json['version'])
        if opts['reboot']
          @log.info(
            "#{r}: their version #{json['version']} is higher than mine #{VERSION}, reboot! " \
            '(use --never-reboot to avoid this from happening)'
          )
          terminate
        end
        @log.debug(
          "#{r}: their version #{json['version']} is higher than mine #{VERSION}, " \
      ...
      if mine < Semantic::Version.new(Zold::Gem.new.last_version)
        if opts['reboot']
          @log.info(
            "#{r}: the version of the gem is higher than mine #{VERSION}, reboot! " \
            '(use --never-reboot to avoid this from happening)'
          )
          terminate
        end
        @log.debug(
          "#{r}: gem version is higher than mine #{VERSION}, " \
          "it's recommended to reboot, but I don't do it because of --never-reboot"
        )
      end
    ...
    def select(opts)
      ...
      @log.info(
        "#{@remotes.all.count} best remote nodes were selected to stay in the list " \
        "(#{@remotes.all.count { |r| r[:master] }} masters)"
      )
    end
    ...
    def ping(host, port, opts)
      ...
      unless opts['ignore-ping']
        raise(RuntimeError, "The node #{host}:#{port} is not responding, #{res.status}:#{res.status_line}")
      end
