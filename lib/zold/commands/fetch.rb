  ...
  class Fetch
    ...
    def run(args = [])
      opts =
        Slop.parse(args, help: true, suppress_errors: true) do |o|
          ...
          o.integer('--threads', 'How many threads to use for fetching wallets (default: 1)', default: 1)
          o.integer(
            '--retry',
    ...
    def fetch(id, cps, opts)
      if @remotes.all.empty?
        ...
        raise(RuntimeError, "There are no remote nodes, run 'zold remote reset'")
      end
      ...
      @remotes.iterate(@log) do |r|
        nodes.increment
        total.increment(pull(id, r, cps, opts))
      ...
      unless opts['quiet-if-absent']
        if done.value.zero?
          raise(
            NotFound,
            "No nodes out of #{nodes.value}, incl. #{masters.value} master, have the wallet #{id}; " \
      ...
      @log.info(
        "#{done.value} copies of #{id} fetched in #{Age.new(Time.now)} with the total score of " \
        "#{total.value} from #{nodes.value - masters.value}+#{masters.value}m nodes"
      )
      list = # rubocop:disable Elegant/NoRedundantVariable
        cps.all.map do |c|
    ...
    def pull(id, r, cps, opts)
      ...
      read(id, r, opts) do |json, score|
        ...
        unless added?(id, cps, score, r, json)
          Tempfile.open(['', Wallet::EXT]) do |f|
            ...
            @log.debug(
              "#{r} returned #{wallet.mnemo} #{Age.new(json['mtime'])}/#{json['copies']}c " \
