...
module Zold
  ...
  class AsyncEntrance
    ...
    def to_json(*_args)
      @entrance.to_json.merge(queue: @queue.size, threads: @pool.count, queue_limit: @limit)
    end
    ...
    def start
      ...
      DirItems.new(@dir).fetch.each do |f|
        ...
        if /^[0-9a-f]{16}-/.match?(f)
          @queue << { id: Id.new(f.split('-').first), file: file }
        else
          File.delete(file)
        end
      end
    ...
    def push(id, body)
      ...
      unless exists?(id, body)
        loop do
          file = File.join(@dir, "#{id}-#{SecureRandom.uuid}#{Wallet::EXT}")
          next if File.exist?(file)
          File.write(file, body)
          @queue << { id: id, file: file }
          @log.debug(
            "Added #{id}/#{Size.new(body.length)} to the queue at pos.#{@queue.size} " \
    ...
    def exists?(id, body)
      DirItems.new(@dir).fetch.each do |f|
        ...
        return true if read(File.join(@dir, f)) == body
      end
      false
    end
    ...
    def take
      ...
      @log.debug(
        "Pushed #{item[:id]}/#{Size.new(body.length)} to #{@entrance.class.name} " \
        "in #{Age.new(Time.now, limit: 0.1)}#{"(#{@queue.size} still in the queue)" unless @queue.empty?}"
      )
    end
  end
end
