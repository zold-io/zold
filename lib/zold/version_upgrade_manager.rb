...
module Zold
  ...
  class VersionUpgradeManager
    # @todo #285 Refactor so we don't just assume there's .zoldata in the current directory.
    #
    # @todo #285 Communicate the ongoing upgrade and its steps to the user.
    #  The user should know:
    #  - If the upgrade is happening.
    #  - What upgrade scripts are run.
    #  - On failure there should be a report of what failed.
    #
    # @todo #285 Write the upgrade manager tests that ensure:
    ...
    def self.data_version
      ...
      # @todo #285 Save version if there is no .zoldata/version yet
      #  This is breaking the specs, it needs additional time.
      # File.open('.zoldata/version', 'w') do |file|
      #   file.puts(Zold::VERSION)
      # end
    ...
    # @todo #285 Compare from_version with the current version and run all the scripts
